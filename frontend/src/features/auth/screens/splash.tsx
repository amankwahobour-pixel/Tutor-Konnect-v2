...
export default function SplashScreen() {
  ...
  const cardScale = useRef(new Animated.Value(0.92));
  ...
  useEffect(() => {
    ...
    Animated.parallel([
      Animated.timing(cardOpacity.current, { toValue: 1, duration: 400, useNativeDriver: true }),
      Animated.spring(cardScale.current, { toValue: 1, tension: 40, friction: 8, useNativeDriver: true }),
    ]).start();
  ...
  return (
    <ImageBackground source={require('@/assets/images/splash.png')} resizeMode="cover" style={styles.background}>
      ...
      <Animated.View style={{ opacity: cardOpacity.current, transform: [{ scale: cardScale.current }], width: '100%', alignItems: 'center' }}>
